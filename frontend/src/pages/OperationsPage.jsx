...
const ACTIONS = ["Dispatch team", "Close valve", "Isolate pipeline", "Increase treatment", "Schedule inspection", "Resolve incident"];
...
export default function OperationsPage() {
  ...
  const apply = (action) => {
    setRows((list) => list.map((r) => (r.id === picked ? { ...r, action, status: action.startsWith("Resolve") ? "CLOSED" : "UPDATED" } : r)));
  };
  return (
        ...
        <ul className="ops-list">
          {rows.map((r) => (
            <li key={r.id} className={picked === r.id ? "selected" : ""} onClick={() => setPicked(r.id)}>
              ...
              <span>{r.action} · {r.status}</span>
            </li>
          ))}
        </ul>
      </Card>
      <Card eyebrow={current.id} title={current.incident}>
        <p>Owner <b>{current.owner}</b></p>
        <p>Current action <b>{current.action}</b></p>
        <div className="action-row">
          {ACTIONS.map((a) => (
            <button key={a} type="button" className="ghost-btn" onClick={() => apply(a)}>{a}</button>
          ))}
        </div>
